...
`production/material-use.schema.json` exposes `plannedMaterialUse` and `actualMaterialUse` as distinct closed contextual-use objects. A planned use references material identity and may carry a planned quantity plus process-context references and use-specific text. An actual use requires the consumed quantity, may reference the specific material lot when known, and may link back to the corresponding planned use. Inventory mutation is not implied by an actual-use record. Timing and trigger structures are intentionally deferred to the process model rather than encoded as ad hoc strings.
